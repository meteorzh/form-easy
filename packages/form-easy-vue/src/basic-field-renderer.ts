...
import { defaultVueFieldComponents } from './basic/default-components';
...
function renderDefaultField(context: BasicFieldRenderContext): VNode {
  ...
  const onChange = (event: Event) =>
    context.onChange(getChangedValue(event, dataType));
  if (dataType === 'boolean') {
    return h('input', {
      type: 'checkbox',
...
export function createVueBasicFieldRenderer(): VueBasicFieldRenderer {
  ...
  return {
    ...
    render(host: HTMLElement, context: BasicFieldRenderContext): void {
      ...
      render(
        component
          ? renderVueComponent(component, context)
          : renderDefaultField(context),
        host
      );
    },
    /** 卸载宿主元素中的 Vue 视图。 */
